...
const Feedback = () => {
  ...
  useEffect(() => {
    if (qTwo !== null) {
      const value = qTwo === 1 ? 'There are privacy settings or permissions that seem excessive or insufficient' : 'There are not privacy settings or permissions that seem excessive or insufficient';
      setPrivacy(value);
    }
  }, [qTwo]);
...
try {
      const response = await axios.post('http://192.168.43.51/Feedback/add-feedback', {
        userid: userId,
